...
def smoothMPASVariable():
    ...
    while iter_num < int(smooth_iter_num):
        ...
        print("\n{} smoothing in total {} iters".format(iter_num,  smooth_iter_num))
...
# === Call functions to perform conversion, extrapolation, and smoothing =============
# Define the variable(s) you are converting. Currently supports a single variable, comma-separated list of variables (no space), or "all"
var_names = []
...
for var_name in var_names:
    #set appropriate options for smoothing and extrapolation
    if var_name == "beta":
        smooth_iter_num = 3
        mask_scheme = "grd"
        extrapolation = "min"
    ...
    # Temperature is not extrapolated, and is therefore zero K outside of the ice mask.
    # If you choose to run without evolving temperature, then you will run into issues
    # if the ice advances into a cell with 0K temperature. However, in general
    # a temperature extrapolation should not be trusted because of the large temperature
    # gradients near the ice margin. If you choose to extrapolate
    # temperature, first make sure you understand all these issues, then
    # use mask_scheme = "all" and extrapolation = "idw". However, note that
    # extrapolating (and smoothing) a 3D field will be very slow.
