...
def plot_transect(ds_transect, mpas_field=None, out_filename=None, ax=None,
    ...
    _plot_outline(x, z, ds_transect.validCells, ax, outline_color,
                  linewidth)
    ...
    for var_name in variable_list:
        ...
        for transect_name, ds_transect in transects.items():
            ...
            _plot_feature_transect(ds_transect, var, var_name, transect_name,
                                   cmap, fc_transects[transect_name],
                                   write_netcdf, method)
...
def plot_feature_transects_main():
    ...
    parser.add_argument('--write_netcdf', dest='write_netcdf',
                        action='store_true',
...
def _plot_outline(x, z, valid_cells, ax, outline_color, linewidth,
                  epsilon=1e-6):
    if outline_color is not None:
        ...
        valid = np.zeros((2 * valid_cells.shape[0] + 2,
                          2 * valid_cells.shape[1] + 2),
                         dtype=float)
...
def _compute_feature_transects(fc, ds_mesh, flip):
    ...
    for transect in fc.features:
        ...
        ds_mpas_transect = compute_transect(x=transect_lon,
                                            y=transect_lat,
...
def _plot_feature_transect(ds_transect, mpas_field, var_name, transect_name,
                           cmap, fc, write_netcdf, method):
    """
    plot a transect showing the field on the MPAS-Ocean mesh and save to a file
    """
