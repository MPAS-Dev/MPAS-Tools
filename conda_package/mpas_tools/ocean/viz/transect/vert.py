...
def find_transect_levels_and_weights(ds_horiz_transect, layer_thickness,
    ...
    (z_half_interface, ssh, z_seafloor, interp_cell_indices,
     interp_cell_weights, valid_transect_cells,
     level_indices) = _get_vertical_coordinate(
    ...
    d_cell_boundary, z_cell_boundary = _get_cell_boundary_segments(
        ssh, z_seafloor, ds_transect_cells.dNode,
    ...
    interp_level_indices, interp_cell_weights, valid_nodes = \
        _get_interp_indices_and_weights(layer_thickness, interp_cell_indices,
...
def _get_vertical_coordinate(ds_transect, layer_thickness, bottom_depth,
    ...
    cell_mask_interp = _get_cell_mask(interp_horiz_cell_indices,
                                      min_level_cell, max_level_cell,
                                      n_vert_levels)
    layer_thickness_interp = layer_thickness_interp.where(cell_mask_interp, 0.)
    ...
    interp_cell_weights = \
        (interp_cell_weights / weight_sum).where(valid_weights)

    layer_thickness_transect = (layer_thickness_interp *
                                interp_cell_weights).sum(dim='nHorizWeights')
    ...
    interp_horiz_cell_weights = \
        (interp_horiz_cell_weights / weight_sum).where(interp_mask)

    ssh_transect = (ssh_interp *
                    interp_horiz_cell_weights).sum(dim='nHorizWeights')
    ...
    return (z_half_interface, ssh_transect, z_seafloor,
            interp_horiz_cell_indices, interp_cell_weights,
            valid_transect_cells, level_indices)
