...
def create_from_generic_mpas_grid():
    """
    ...
    parser = OptionParser()
    parser.add_option(
       "-i", "--in", dest="fileinName",
       help="input filename.  Defaults to 'grid.nc'",
       metavar="FILENAME")
    parser.add_option(
       "-o", "--out", dest="fileoutName",
    ...
    parser.add_option(
       ...
       default='glimmer', metavar="FILENAME")
    parser.add_option(
       "--beta", dest="beta", action="store_true",
       help="DEPRECATED")
    parser.add_option(
       "--effecpress", dest="effecpress", action="store_true",
       help="DEPRECATED")
    parser.add_option(
       "--diri", dest="dirichlet", action="store_true",
    ...
    parser.add_option(
       "--thermal", dest="thermal", action="store_true",
    ...
    parser.add_option(
       "--hydro", dest="hydro", action="store_true",
    ...
    parser.add_option(
       "--obs", dest="obs", action="store_true",
        ...
        if dim == 'nTracers':
            ...
            if dim == 'Time':
                ...
                if options.levels is None:
                    ...
                    print(f"Using nVertLevels from the intput file: "
                    ...
                    # value
                    print(f"Using nVertLevels specified on the command line: "
    ...
    if 'nVertLevels' not in fileout.dimensions:
        if options.levels is None:
            print("nVertLevels not in input file and not specified.  Using "
                  "default value of 10.")
    ...
    fileout.createDimension('nVertInterfaces',
                            len(fileout.dimensions['nVertLevels']) + 1)
    print(f'Added new dimension nVertInterfaces to output file with value of '
    ...
    vars2copy = ['latCell', 'lonCell', 'xCell', 'yCell', 'zCell',
                 'indexToCellID', 'latEdge', 'lonEdge', 'xEdge', 'yEdge',
                 'zEdge', 'indexToEdgeID', 'latVertex', 'lonVertex', 'xVertex',
                 'yVertex', 'zVertex', 'indexToVertexID', 'cellsOnEdge',
                 'nEdgesOnCell', 'nEdgesOnEdge', 'edgesOnCell', 'edgesOnEdge',
                 'weightsOnEdge', 'dvEdge', 'dcEdge', 'angleEdge', 'areaCell',
                 'areaTriangle', 'cellsOnCell', 'verticesOnCell',
                 'verticesOnEdge', 'edgesOnVertex', 'cellsOnVertex',
                 'kiteAreasOnVertex']
    # Add these optional fields if they exist in the input file
    for optionalVar in ['meshDensity', 'gridSpacing', 'cellQuality',
                        'triangleQuality', 'triangleAngleQuality',
                        'obtuseTriangle']:
    ...
    for varname in vars2copy:
        ...
        if filein.on_a_sphere == "YES             ":
            if varname in ('xCell', 'yCell', 'zCell', 'xEdge', 'yEdge',
                           'zEdge', 'xVertex', 'yVertex', 'zVertex', 'dvEdge',
                           'dcEdge'):
    ...
    layerThicknessFractions = fileout.createVariable('layerThicknessFractions',
                                                     datatype,
                                                     ('nVertLevels', ))
    ...
    if options.vertMethod == 'uniform':
        ...
        print(f"Setting layerThicknessFractions to: "
              f"{layerThicknessFractionsData}")
    else:
        raise ValueError(f'Unknown method for vertical spacing method '
    ...
    # to state.
    newvar = fileout.createVariable('bedTopography', datatype,
                                    ('Time', 'nCells'))
    newvar[:] = numpy.zeros(newvar.shape)
    newvar = fileout.createVariable('sfcMassBal', datatype,
                                    ('Time', 'nCells'))
    newvar[:] = numpy.zeros(newvar.shape)
    newvar = fileout.createVariable('floatingBasalMassBal', datatype,
                                    ('Time', 'nCells'))
    newvar[:] = numpy.zeros(newvar.shape)
    print('Added default variables: thickness, temperature, bedTopography, '
          'sfcMassBal, floatingBasalMassBal')

    newvar = fileout.createVariable('beta', datatype,
                                    ('Time', 'nCells'))
    # Give a default beta that won't have much sliding.
    newvar[:] = 1.0e8
    print('Added variable: beta')

    newvar = fileout.createVariable('muFriction', datatype,
                                    ('Time', 'nCells'))
    # Give a default mu that won't have much sliding.
    newvar[:] = 1.0e8
    print('Added variable: muFriction')

    newvar = fileout.createVariable('effectivePressure', datatype,
                                    ('Time', 'nCells'))
    ...
    newvar = fileout.createVariable('stiffnessFactor', datatype,
                                    ('Time', 'nCells'))
    # Give default value
    newvar[:] = 1.0
    print('Added variable: stiffnessFactor')

    newvar = fileout.createVariable('eigencalvingParameter', datatype,
                                    ('Time', 'nCells'))
    # Give default value for eigencalvingParameter
    newvar[:] = 3.14e16
    print('Added variable: eigencalvingParameter')

    newvar = fileout.createVariable('groundedVonMisesThresholdStress',
                                    datatype, ('Time', 'nCells'))
    # Give default value
    newvar[:] = 1.0e6
    print('Added variable: groundedVonMisesThresholdStress')

    newvar = fileout.createVariable('floatingVonMisesThresholdStress',
                                    datatype, ('Time', 'nCells'))
    # Give default value
    newvar[:] = 1.0e6
    print('Added variable: floatingVonMisesThresholdStress')

    newvar = fileout.createVariable('iceMask', datatype,
                                    ('Time', 'nCells'))
    newvar[:] = 0
    print('Added variable: iceMask')

    if options.dirichlet:
        newvar = fileout.createVariable('dirichletVelocityMask', datatypeInt,
                                        ('Time', 'nCells', 'nVertInterfaces'))
        # default: no Dirichlet b.c.
        newvar[:] = 0
        newvar = fileout.createVariable('uReconstructX', datatype,
                                        ('Time', 'nCells', 'nVertInterfaces',))
        newvar[:] = 0.0
        newvar = fileout.createVariable('uReconstructY', datatype,
                                        ('Time', 'nCells', 'nVertInterfaces',))
        newvar[:] = 0.0
        print('Added optional dirichlet variables: dirichletVelocityMask, '
              'uReconstructX, uReconstructY')

    if options.thermal:
        newvar = fileout.createVariable('temperature', datatype,
                                        ('Time', 'nCells', 'nVertLevels'))
        # Give default value for temperate ice
        newvar[:] = 273.15
        newvar = fileout.createVariable('surfaceAirTemperature', datatype,
                                        ('Time', 'nCells'))
        # Give default value for temperate ice
        newvar[:] = 273.15
        newvar = fileout.createVariable('basalHeatFlux', datatype,
                                        ('Time', 'nCells'))
        # Default to none (W/m2)
        newvar[:] = 0.0
        print('Added optional thermal variables: temperature, '
              'surfaceAirTemperature, basalHeatFlux')

    if options.hydro:
        newvar = fileout.createVariable('waterThickness', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('tillWaterThickness', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('basalMeltInput', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('externalWaterInput', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('frictionAngle', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('waterPressure', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('waterFluxMask', 'i',
                                        ('Time', 'nEdges'))
        newvar[:] = 0.0
        print('Added optional hydro variables: waterThickness, '
              'tillWaterThickness, meltInput, frictionAngle, waterPressure, '
              'waterFluxMask')

    if options.obs:
        newvar = fileout.createVariable('observedSurfaceVelocityX', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('observedSurfaceVelocityY', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('observedSurfaceVelocityUncertainty',
                                        datatype, ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('observedThicknessTendency', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('observedThicknessTendencyUncertainty',
                                        datatype, ('Time', 'nCells'))
        newvar[:] = 0.0
        newvar = fileout.createVariable('thicknessUncertainty', datatype,
                                        ('Time', 'nCells'))
        newvar[:] = 0.0
        print('Added optional velocity optimization variables: '
              'observedSurfaceVelocityX, observedSurfaceVelocityY, '
              'observedSurfaceVelocityUncertainty, observedThicknessTendency, '
              'observedThicknessTendencyUncertainty, thicknessUncertainty')
    ...
    print("Completed creating land ice variables in new file. Now syncing to "
          "file.")
    fileout.sync()
