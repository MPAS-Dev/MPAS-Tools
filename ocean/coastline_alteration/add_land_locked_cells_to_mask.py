...
parser.add_argument("-n", "--number_sweeps", dest="nSweeps",
                    help="Maximum number of sweeps to search for land-locked "
                         "cells.",
...
for iCell in range(nCells):
    ...
    if abs(latCell[iCell]) < latitude_threshold_radians or \
            landMask[iCell] == 1:
        continue
...
for iRemovableCell in range(0, nRemovableCells):
    ...
    if (landMaskDiagnostic[iCell] == 0):
        ...
        if (latCell[iCell] > 84.0 * d2r  # North Pole
                # Arctic
                or lonCell[iCell] > 160.0 * d2r
                and lonCell[iCell] < 230.0 * d2r
                and latCell[iCell] > 73.0 * d2r
                # North Atlantic
                or lonCell[iCell] > 315.0 * d2r
                and lonCell[iCell] < 340.0 * d2r
                and latCell[iCell] > 15.0 * d2r
                and latCell[iCell] < 45.0 * d2r
                # North Atlantic
                or lonCell[iCell] > 290.0 * d2r
                and lonCell[iCell] < 300.0 * d2r
                and latCell[iCell] > 72.0 * d2r
                and latCell[iCell] < 75.0 * d2r
                # North Atlantic 2
                or lonCell[iCell] > 0.0 * d2r
                and lonCell[iCell] < 10.0 * d2r
                and latCell[iCell] > 70.0 * d2r
                and latCell[iCell] < 75.0 * d2r
                # North Pacific
                or lonCell[iCell] > 150.0 * d2r
                and lonCell[iCell] < 225.0 * d2r
                and latCell[iCell] > 0.0 * d2r
                and latCell[iCell] < 45.0 * d2r
                # South Atlantic
                or lonCell[iCell] > 0.0 * d2r
                and lonCell[iCell] < 5.0 * d2r
                and latCell[iCell] > -60.0 * d2r
                and latCell[iCell] < 0.0 * d2r
                # South Pacific
                or lonCell[iCell] > 180.0 * d2r
                and lonCell[iCell] < 280.0 * d2r
                and latCell[iCell] > -60.0 * d2r
                and latCell[iCell] < -10.0 * d2r
                # Southern Ocean
                or lonCell[iCell] > 0.0 * d2r
                and lonCell[iCell] < 165.0 * d2r
                and latCell[iCell] > -60.0 * d2r
                and latCell[iCell] < -45.0 * d2r):
            floodFill[iCell] = 1
        ...
        if landMaskDiagnostic[iCell] == 3:
            for iEdgeOnCell in range(nEdgesOnCell[iCell]):
                ...
                if oceanMask[cellsOnCell[iCell, iEdgeOnCell] - 1] == 1:
                    ...
                    iP1 = (iEdgeOnCell + 1) % nEdgesOnCell[iCell]
                    iM1 = (iEdgeOnCell - 1) % nEdgesOnCell[iCell]
                    # Is either of this neighbor's two neighbors to left and
                    # right ocean?
                    # if so, sum of masks is two.
                    # subtract 1 to convert 1-base to 0-base:
                    if (landMask[cellsOnCell[iCell, iP1] - 1] == 0
                        ...
                        oceanMask[iCell] = 1
                        # once we remove this cell, we can quit checking over
                        # edges
                        break
