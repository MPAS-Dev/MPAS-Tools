...
!gfortran smooth_topo_before_init.F90 -o a.out -I /share/apps/netcdf/4.6.3/gcc/4.8.5/include/ -L /share/apps/netcdf/4.6.3/gcc/4.8.5/lib/ -lnetcdf -lnetcdff
  ...
  integer :: &
       numSmoothingPasses, smoothingPass
...
!   status = nf90_create(trim(filename), NF90_CLOBBER, ncid)
    ...
    status = nf90_put_att(ncid, NF90_GLOBAL, "input_land_ice_mask_file", trim(filename_land_ice_mask))
    call netcdf_error(status, "create_depth_file: nf90_put_att input_land_ice_mask_file")
  ...
  subroutine load_land_ice_mask_file()
    ...
    write(*,*) "Load land-ice mask file..."

    ! open file
    status = nf90_open(trim(filename_land_ice_mask), NF90_NOWRITE, ncid)
    call netcdf_error(status, "load_land_ice_mask_file: nf90_open")

    allocate(landIceMask(nCells))

    ! bedElevation
    status = nf90_inq_varid(ncid, "landIceMask", varid)
    call netcdf_error(status, "load_land_ice_mask_file: nf90_inq_varid landIceMask")

    status = nf90_get_var(ncid, varid, landIceMask)
    call netcdf_error(status, "load_land_ice_mask_file: nf90_get_var landIceMask")

    ! close
    status = nf90_close(ncid)
    call netcdf_error(status, "load_land_ice_mask_file: nf90_close")

  end subroutine load_land_ice_mask_file
