...
file_list = sorted(glob.glob(args.filename_pattern))
time_indices = []
time_file = []
time_global_indices = []
...
for file in file_list:
    ...
    for time_idx in np.arange(0, times):
        ...
        time_file.append(file)
        time_global_indices.append(i_global)
...
for time_index in time_global_indices:
    ...
    for iBlock in np.arange(0, nBlocks):
        ...
        if field_ndims == 2:
            field = field_var[time_indices[time_idx], blockStart:blockEnd]
        elif field_ndims == 3:
            field = field_var[time_indices[time_idx], blockStart:blockEnd, 0]
        elif field_ndims == 4:
            field = field_var[time_indices[time_idx], blockStart:blockEnd, dim_vals[2], dim_vals[3]]
        elif field_ndims == 5:
            field = field_var[time_indices[time_idx], blockStart:blockEnd, dim_vals[2], dim_vals[3], dim_vals[4]]
